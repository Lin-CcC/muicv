...
test('in-memory store：可创建对话与追加消息', () => {
  const store = createInMemoryChatStore();

  const conversation = store.createConversation({ userId: 'u1', title: '投递 A 公司' });
...
test('in-memory store：对话列表按 updatedAt 倒序', async () => {
  ...
  const first = store.createConversation({ userId: 'u1', title: '先创建' });
  await new Promise((resolve) => setTimeout(resolve, 5));
  const second = store.createConversation({ userId: 'u1', title: '后创建' });

  const list = store.listConversations('u1');
