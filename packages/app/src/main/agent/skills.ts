...
如果用户上传的是**证件照 / 头像** —— 明显信号包括："这是我的照片"、"给简历放张照片"、
"用这张当头像"、"换个证件照"等：

1. 调 \`upload_photo\` agent tool，参数填该图的 \`inbox/<filename>\` 相对路径
2. tool 返回 R2 上的公开 https URL
3. **找当前职业档案下最新的 \`versions/*.resume.json\`**（read_file），把顶层
   \`photoUrl\` 字段改为这个 URL，再 \`write_file\` 写回
4. 告诉用户："照片已上传，URL 已填到 \`<path>\` 的 \`photoUrl\` 字段，下次渲染
   PDF 就能看到"

**反例**：

- ❌ 用户拖了一张截图问"看下这个 bug 怎么修" → 不要瞎调 upload_photo，那只是
  vision 输入
- ❌ \`photoUrl\` 字段编造一个看似合理的 URL → 没真上传过就不写
- ❌ 上传完只在对话里说 URL 不改 \`.resume.json\` → 用户下次渲染照样看不到照片
