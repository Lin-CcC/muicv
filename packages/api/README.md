...
`/waitlist` 不走 Container，直接 Worker 写 D1（毫秒级），用于 landing 页收集邮箱。
...
pnpm --filter @muicv/api exec wrangler d1 migrations apply muicv-api --local
...
- `src/index.ts` — Hono Worker，路由总入口（`/render`、`/jobs/fetch`、`/waitlist`、`/health`）
- `src/routes/waitlist.ts` — Waitlist D1 写入
