...
- **域名**：生产用 `meathill.com` 已有 zone，产品挂子域：
  - `muicv.meathill.com` — 营销站（`packages/website`）
  - `app.muicv.meathill.com` — 产品后台（`packages/app`，M2 起激活）
  - `api.muicv.meathill.com` — API（`packages/api`）
...
**绑定域名 `api.muicv.meathill.com`**：

编辑 `packages/api/wrangler.jsonc`，把注释掉的 `routes` 节解开：

```jsonc
"routes": [
  { "pattern": "api.muicv.meathill.com/*", "zone_name": "meathill.com" }
]
```

前提：Cloudflare DNS 里 `meathill.com` zone 已有 `api.muicv` 的 proxied CNAME 或 A 记录。再 `pnpm --filter @muicv/api deploy` 即可生效。
...
- 接入 **Better Auth**（决定账号方案）
- 设计 users / subscriptions / usage_logs 表 + 对应 migrations（和 `packages/api` 的 D1 共用还是独立库，M2 时决定）
